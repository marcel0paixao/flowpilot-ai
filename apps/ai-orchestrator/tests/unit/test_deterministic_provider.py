...
def test_deterministic_provider_returns_stable_prompt_result() -> None:
    ...
    first_result = provider.run(config=config, input_data=input_data)
    second_result = provider.run(config=config, input_data=input_data)
...
def test_deterministic_provider_uses_prompt_config_defaults() -> None:
    ...
    result = provider.run(config=config, input_data={"ticketId": "ticket-1"})
...
def test_deterministic_provider_handles_empty_input() -> None:
    ...
    result = provider.run(config=config, input_data={})
