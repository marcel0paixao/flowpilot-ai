...
- Smoke-test the new web app in a browser against the local Docker/API stack.
- Add focused frontend tests for auth routing, token handling, workspace list/create, workflow list/detail, run action, and execution summary polling.
- Improve API error states and empty states across the new frontend screens.
- Refine the React Flow layout for larger DAGs and parallel branches.
- Add editable workflow definition support: drag nodes, connect edges, inspector editing, validation feedback, and save version.
- Decide whether execution updates should stay with polling for MVP or move to SSE/WebSocket later.
...
Além disso, `apps/web` já foi convertido para React + Vite + TypeScript com Tailwind CSS, shadcn/ui-style local components, React Router, TanStack Query, JWT auth, app shell, workspaces, workflows, workflow detail, React Flow read-only canvas, run action, executions overview e execution detail com polling no summary endpoint.

Depois disso, me ajude a continuar a Semana 1. Quero que você atue como tech lead/coding partner: primeiro rode `git status`, revise o estado atual, suba/valide a stack com `docker compose up -d` ou `docker compose ps`, rode `pnpm --filter @flowpilot/api seed:demo` se necessário, rode os checks viáveis, e então faça smoke-test/polimento do front-end MVP: login, workspace context, workflow list/detail, execução de workflow, React Flow read-only e tela de execution summary com polling. Próximos passos técnicos prováveis: testes frontend focados, estados de erro/loading, refinamento do layout do canvas e início do editor visual. Atualize `docs/STATUS.md`, `docs/DECISIONS.md` e `docs/NEXT_STEPS.md` ao final.
